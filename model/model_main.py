...
from .my_roi_aligh import RoIPoolTrans

class IQARegression(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.cnnExtractor = RoIPoolTrans()
    ...
    def forward(self, enc_inputs, x):
        enc_inputs_embed=self.cnnExtractor(x)
        #enc_inputs_embed  = enc_inputs_embed.transpose(1,2) #B N D->B D N
        #enc_outputs=self.pool(enc_inputs_embed) #B D N ->B D 1
        #enc_outputs = enc_outputs.view(enc_outputs.size(0),-1) #B D 1->B D
        #enc_outputs = enc_inputs_embed.view(enc_inputs_embed.size(0),-1) # no patch no trans
        #pred = self.projection2(enc_outputs) #B D->B 1
        ...
        enc_outputs = self.transformer(enc_inputs, enc_inputs_embed)
...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_inputs_embed):
...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, self.config.n_enc_seq+1, self.config.d_hidn)) #xixi 1,29x29+1,256
